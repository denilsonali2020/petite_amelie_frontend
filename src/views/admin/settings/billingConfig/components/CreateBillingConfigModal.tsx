...
import CreateBillingConfigModalForm from "./CreateBillingConfigModalForm";

export default function CreateBillingConfigModal() {
  const location = useLocation();
  const navigate = useNavigate();
  const queryParams = new URLSearchParams(location.search);
  const show = queryParams.has("newBillingConfig");
                  ...
                  <div className="mb-8 border-l-4 border-pink-600 pl-4 py-1">
                    ...
                    <div className="mt-1">
                      <p className="text-[11px] font-medium uppercase tracking-widest text-slate-400/80">
                        Registra los datos de facturación autorizados.
                      </p>
                    </div>
                  </div>
                  {/* FORMULARIO */}
                  <CreateBillingConfigModalForm closeModal={closeModal} />
                </>
              </DialogPanel>
            </TransitionChild>
          </div>
