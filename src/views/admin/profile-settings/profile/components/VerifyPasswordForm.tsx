...
import { verifyPassword } from "@/views/admin/user/services/userService";
...
export default function VerifyPasswordForm() {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: () => {
      reset();
      navigate(`${location.pathname}?changePin=true`, { replace: true });
    },
    onError: (error) => toast.error(error.message, { id: "verifyError" }),
  });
  ...
  return (
    <div className="bg-white shadow-sm ring-1 ring-gray-200 rounded-xl p-6 sm:p-8">
      <div className="border-b border-gray-100 pb-5 mb-6">
        <h2 className="text-lg font-semibold text-gray-900">PIN de Ventas</h2>
        <p className="mt-1 text-sm text-gray-400">
          Para cambiar tu código rápido de 4 dígitos, primero verifica tu
          identidad.
        </p>
      </div>
      ...
      <form className="space-y-5 max-w-md" onSubmit={handleSubmit(onSubmit)}>
        <div>
          ...
          <div className="relative">
            <input
              ...
              {...register("password", {
                required: "Debes ingresar tu contraseña para continuar",
              })}
            />
            <button
              type="button"
        ...
        <button
          type="submit"
          disabled={isPending}
          className="w-full sm:w-auto rounded-lg bg-pink-600 px-6 py-2.5 text-sm font-bold text-white shadow-md hover:bg-pink-700 disabled:opacity-50 cursor-pointer transition-all active:scale-95"
        >
          {isPending ? "Verificando..." : "Verificar y cambiar PIN"}
        </button>
      </form>
    </div>
  );
